...
constexpr uint32_t FEX_STATS_VERSION = 1;
...
void update_fex_stats() {
    ...
    for (auto header_offset = __atomic_load_n(header_offset_atomic, __ATOMIC_RELAXED);
        ...
        if (TID != 0) {
            ...
            memcpy(&sampled_stats.previous, &sampled_stats.current, sizeof(fex_thread_stats));
            memcpy(&sampled_stats.current, (const void*)stat, sizeof(fex_thread_stats));
